...
const loginControl = (request, response) => {
    ...
    if (!username || !password) {
        response.render('failedLogin', { msg: "Unable to login, Please try after some time!" });
    } else {
...
const registerControl = (request, response) => {
    ...
    clientServices.registerService(client, function(err, exists, insertedID) {
        ...
        if (exists) {
            response.render('postRegister', { msg: `Registeration Failed` });

        } else {
            client.num_client = insertedID;
            console.log(`Registration (${username}, ${insertedID}) successful!`);
            response.render('postRegister', { msg: `Registration Successful ${username}!` });
        }
        response.end();
    });
};
